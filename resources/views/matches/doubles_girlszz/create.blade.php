...
<div class="container">
    ...
    <form method="POST">
        ...
        <select name="tournament_id" id="tournament_id" required {{ $lockedTournamentId ? 'disabled' : '' }}>
        <option value="">Select Tournament</option>
            @foreach($tournaments as $tournament)
                <option value="{{ $tournament->id }}" {{ $lockedTournament && $lockedTournament->id == $tournament->id ? 'selected' : '' }}>
                    {{ $tournament->name }}
                </option>
            @endforeach
        </select>
        ...
        @if($lockedTournament)
            <button type="submit" formaction="{{ route('matches.doubles_girls.unlockTournament') }}" class="btn btn-danger">Unlock Tournament</button>
        @else
            <button type="submit" formaction="{{ route('matches.doubles_girls.lockTournament') }}" class="btn btn-primary">Lock Tournament</button>
        @endif
    </form>

    <!-- Girls Doubles Match Form (Only when Tournament is Locked) -->
    @if($lockedTournament)
    <form method="POST" action="{{ route('matches.doubles_girls.store') }}">
        ...
        <label for="time">Match Time (24-hour format HH:MM):</label>
        ...
        @for ($i = 1; $i <= 3; $i++)
            ...
            <input type="number" name="set{{ $i }}_team1_points" id="set{{ $i }}_team1_points" required>

            <label for="set{{ $i }}_team2_points">Set {{ $i }} Team 2 Points:</label>
            <input type="number" name="set{{ $i }}_team2_points" id="set{{ $i }}_team2_points" required>
        @endfor
...
<script>
    ...
    document.getElementById('category_id').addEventListener('change', updatePlayerDropdown);
</script>
@endsection
